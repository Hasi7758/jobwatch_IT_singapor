...
## 页面怎么看
- **点标签就能筛选**:顶部三组标签(方向 / 经验 / 行业),同组内是"或",跨组是"且"。
  比如点「云/DevOps/平台」+「德企/德语区」= 德企的云岗;职位卡片上的标签也能点。
- 搜索框按职位名/公司名过滤;「隐藏中介/外包岗」一键去掉派遣商;「清除筛选」复位。
- 筛选状态写在网址 `#` 后面,可以把常用组合(如 `#t=云/DevOps/平台&na=1`)收藏成书签。
