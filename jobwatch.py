#!/usr/bin/env python3
"""
jobwatch_IT_singapor
====================
每天抓一次新加坡的 IT 工程类职位(Cloud / DevOps / Platform / AI / Software Engineer),
只报告"今天第一次见到"的那些。面向 1 年左右工作经验,过滤掉 Senior/Lead 等资深岗和 Data 类岗位。

核心思路:不相信任何平台标注的发布日期(聚合平台经常滞后),
...
def load_config():
    ...
    cfg["location"].setdefault("terms", ["singapore", "sg", "singapura"])
    cfg["location"].setdefault("allow_remote", False)
    cfg["location"].setdefault("plz_prefixes", [])
...
def kw_hit(term, text):
    """整词匹配:关键词前后不能紧贴字母/数字。
    这样 "lead" 不会误伤 "Leadership Programme","data" 不会误伤 "Database"(后者另列),
    "sg" 不会命中 "Glasgow"。关键词本身可以含空格、点、斜杠、加号。"""
    ...
    for term in (mc.get("queries") or [""]):
        for page in range(int(mc.get("pages", 5))):
            ...
            for it in items:
                ...
                # MCF 上的职位全在新加坡;带上街道地址方便看,但末尾固定加 Singapore 保证地点过滤通过
                street = " ".join(str(x) for x in [addr.get("block"), addr.get("street")] if x)
                loc = f"{street}, Singapore" if street else "Singapore"
                out.append(Job(
                    uid="mcf:" + str(jid),
...
ROLE_RULES = [
    ("云/DevOps/平台", [
        ...
        "platform engineer", "platform engineering", "infrastructure", "infra", "kubernetes", "k8s",
        "aws", "azure", "gcp", "release engineer", "build engineer", "ci/cd", "cicd",
        "systems engineer", "system engineer",
    ]),
    ("AI/ML", [
        "ai", "a.i.", "ai/ml", "machine learning", "ml", "mlops", "llm", "genai", "gen ai",
        "generative ai", "deep learning", "nlp", "computer vision", "agentic",
    ]),
    ("软件开发", [
...
JUNIOR_TERMS = [
    "junior", "jr", "jr.", "associate", "graduate", "grad", "new grad", "fresh grad", "fresh graduate",
    "entry level", "entry-level", "early career", "early careers", "young talent", "campus",
    "engineer i", "engineer 1", "developer i", "developer 1", "analyst i", "analyst 1",
    "level 1", "l1", "0-2 years", "1-2 years", "1-3 years", "1 year", "2 years",
]
JUNIOR_TAG = "初级友好"
...
CATEGORY_RULES = [
    ("科技大厂", [
        ...
        "hewlett packard", "hpe", "hp inc", "vmware", "broadcom", "nvidia", "sap", "servicenow",
        "adobe", "autodesk", "zendesk", "atlassian", "twilio", "okta", "cloudflare", "datadog",
        "snowflake", "databricks", "mongodb", "elastic", "hashicorp", "gitlab", "github", "palantir",
        "openai", "anthropic", "equinix", "akamai", "splunk", "workday", "intuit", "paypal",
        "dyson", "motional", "tesla", "bloomberg", "indeed", "expedia", "booking.com",
    ]),
    ("互联网平台", [
        "grab", "shopee", "sea limited", "sea ltd", "garena", "lazada", "carousell", "foodpanda",
        "delivery hero", "ninja van", "agoda", "trip.com", "gojek", "goto", "traveloka",
        "propertyguru", "99.co", "zalora", "razer", "circles.life", "circles", "singtel", "starhub",
        "m1 limited", "mediacorp", "secretlab", "love, bonito", "atome", "shopback", "coupang",
        "tokopedia", "bukalapak", "klook", "redmart", "deliveroo", "lalamove", "j&t", "ryde",
    ]),
    ("金融科技与加密", [
        "stripe", "nium", "xendit", "airwallex", "wise", "transferwise", "revolut", "adyen",
        "coda payments", "aspire", "endowus", "stashaway", "syfe", "thunes", "coinbase",
        "crypto.com", "binance", "okx", "bybit", "kraken", "ripple", "circle", "matrixport",
        "amber group", "2c2p", "fomo pay", "youtrip", "funding societies", "validus", "moneysmart",
        "hashkey", "coinhako", "independent reserve", "bitget", "kucoin", "gate.io", "dtcpay",
        "triple-a", "fazz", "volopay", "spenmo", "grabpay", "chocolate finance", "trust bank",
        "gxs", "maribank", "anext", "green link digital", "partior", "marketnode", "adDX",
        "sygnum", "paxos", "fireblocks", "chainalysis", "tenity", "pine labs",
        "payoneer", "rapyd", "checkout.com", "flywire", "visa", "mastercard", "american express",
    ]),
    ("银行与金融机构", [
        "dbs", "ocbc", "oversea-chinese banking", "uob", "united overseas bank", "standard chartered", "hsbc", "citi",
        "citibank", "citigroup", "citicorp", "jpmorgan", "j.p. morgan", "jp morgan", "goldman", "morgan stanley", "ubs",
        "credit suisse", "deutsche bank", "bnp paribas", "barclays", "societe generale",
        "société générale", "bank of america", "maybank", "cimb", "bank of china", "icbc",
        "mizuho", "mufg", "smbc", "sumitomo", "nomura", "julius baer", "pictet", "lgt", "rbc",
        "natwest", "anz", "westpac", "commonwealth bank", "nab", "sgx", "gic", "temasek",
        "monetary authority", "mas", "prudential", "aia", "great eastern", "income insurance",
        "ntuc income", "manulife", "allianz", "axa", "chubb", "zurich", "swiss re", "munich re",
        "blackrock", "state street", "bny", "northern trust", "franklin templeton", "schroders",
        "fidelity", "vanguard", "invesco", "wellington", "jane street", "citadel", "jump trading",
        "optiver", "imc", "drw", "tower research", "hudson river", "flow traders", "qube",
        "millennium", "point72", "two sigma", "de shaw", "d.e. shaw", "squarepoint", "grasshopper",
        "virtu", "susquehanna", "sig", "akuna", "xtx", "gsr", "wintermute", "tudor", "balyasny",
        "bridgewater", "man group", "marshall wace", "brevan howard", "dymon", "cme", "lseg",
        "refinitiv", "s&p global", "moody's", "msci", "fitch", "ion group", "murex", "finastra",
        "fis", "fiserv", "broadridge", "ss&c", "temenos", "avaloq", "synpulse",
    ]),
    ("政府与公共部门", [
        "govtech", "government technology", "dsta", "defence science", "dso national", "synapxe",
        "ihis", "integrated health information", "cpf", "central provident", "hdb",
        "housing & development", "housing and development", "lta", "land transport", "iras",
        "inland revenue", "mindef", "ministry of", "home team science", "htx", "a*star", "astar",
        "agency for science", "nus", "national university of singapore", "ntu",
        "nanyang technological", "smu", "singapore management university", "sutd", "singapore institute",
        "singapore polytechnic", "nanyang polytechnic", "temasek polytechnic", "republic polytechnic",
        "ngee ann", "ite ", "sp group", "singapore power", "psa", "changi airport", "sats",
        "singapore airlines", "sia ", "smrt", "sbs transit", "comfortdelgro", "sph media",
        "singhealth", "nhg", "national healthcare", "nuhs", "national university health",
        "ttsh", "sgh", "kkh", "st engineering", "singapore technologies", "csit",
        "cyber security agency", "csa", "imda", "infocomm media", "edb", "enterprise singapore",
        "jtc", "ura", "urban redevelopment", "nea", "national environment", "pub", "moe",
        "ministry of education", "moh", "ministry of health", "mha", "mom", "ministry of manpower",
        "acra", "sla", "singapore land authority", "sport singapore", "nparks", "national parks",
        "nlb", "national library", "seab", "skillsfuture", "workforce singapore", "wsg", "e2i",
        "peoples association", "people's association", "mccy", "mci", "mse", "mnd", "mot",
        "mti", "pmo", "prime minister", "smart nation", "opengov", "open government",
        "singapore tourism", "stb", "maritime and port", "mpa", "caas", "civil aviation",
        "bca", "building and construction", "hsa", "health sciences", "sfa", "food agency",
        "avs", "nccs", "national cancer", "nhcs", "national heart", "duke-nus", "lkc",
    ]),
    ("咨询与IT服务", [
        "accenture", "ncs", "tcs", "tata consultancy", "infosys", "wipro", "cognizant", "hcl",
        "tech mahindra", "capgemini", "dxc", "kyndryl", "deloitte", "pwc", "pricewaterhouse",
        "ernst & young", "ey ", "kpmg", "thoughtworks", "ntt data", "nttdata", "fujitsu", "hitachi",
        "avanade", "sopra steria", "atos", "eviden", "globant", "epam", "endava", "nagarro",
        "zensar", "ltimindtree", "mindtree", "mphasis", "virtusa", "persistent", "synechron",
        "luxoft", "adnovum", "trusted services", "nera", "titansoft", "zuhlke", "zühlke", "palo it",
        "palo-it", "amaris", "gft", "thatz", "u3 infotech", "optimum solutions", "nucleus software",
        "sciente", "elitez", "sopra", "mckinsey", "bcg", "boston consulting", "bain", "oliver wyman",
        "publicis sapient", "slalom", "unisys", "cgi", "insight enterprises", "rackspace",
        "softwareone", "crayon", "cloudmile", "searce", "megazone", "eCloudvalley", "ecloudvalley",
        "cloud kinetics", "cloud comrade", "pointstar", "ingram",
    ...
    ("招聘中介", [
        ...
        "aims", "business edge", "capita", "cornerstone", "dynamic human capital", "evo outsourcing",
        "gmp ", "hire digital", "iss consulting", "kerry consulting", "links international",
        "nala employment", "peoplebank", "quess", "recruitfirst", "recruit first", "search personnel",
        "star recruitment", "talentvis", "the supreme hr", "trust recruit", "u ventures",
        "hr consultancy", "recruitment", "recruiters", "staffing", "employment agency", "manpower services",
        "talent solutions", "human capital", "agensi pekerjaan", "jobstreet",
        "glints", "workclass", "achieve group",
        "hays specialist", "people profilers", "the resolute", "randstad sourceright",
        "experis", "rgf professional", "monroe consulting", "spring professional", "lhh", "ntt com",
        "flintex", "helius", "aureus", "octomate", "tempserv", "recruit haus", "scientec",
        "wecruit", "nextbeat", "jobs360",
    ]),
    ("中资出海", [
        "bytedance", "tiktok", "shein", "temu", "pdd", "alibaba", "lazada", "ant group", "alipay",
        "ant international", "tencent", "huawei", "xiaomi", "byd", "oppo", "vivo", "oneplus", "dji",
        "sensetime", "megvii", "meituan", "didi", "jd.com", "jd ", "netease", "bilibili",
        "kuaishou", "baidu", "zte", "hikvision", "lenovo", "haier", "midea", "anker", "trip.com",
        "webull", "futu", "moomoo", "tuya", "agora", "ping an", "onechain", "bitdeer",
        "hisense", "tcl", "boe", "catl", "geely", "nio", "xpeng", "li auto", "zeekr", "leapmotor",
        "ctrip", "qunar", "tongcheng", "vipshop", "xiaohongshu", "rednote",
        "yeahmobi", "mobvista", "cheetah", "apus", "kunlun", "opay", "palmpay", "transsion",
        "unisoc", "spreadtrum", "horizon robotics", "cambricon", "moore threads", "biren",
        "zhipu", "moonshot", "minimax", "deepseek", "manus", "butterfly effect", "hashkey",
    ]),
    ("半导体与硬件", [
        "micron", "globalfoundries", "global foundries", "applied materials", "kla", "lam research",
        "asml", "amd", "advanced micro", "intel", "nvidia", "qualcomm", "mediatek", "marvell",
        "infineon", "stmicro", "st micro", "texas instruments", "analog devices", "nxp", "renesas",
        "microchip", "onsemi", "on semiconductor", "skyworks", "qorvo", "western digital",
        "seagate", "keysight", "national instruments", "siltronic", "ams osram", "osram",
        "soitec", "amkor", "kulicke", "ultra clean", "entegris", "onto innovation", "nanofilm",
        "murata", "tdk", "jabil", "venture corp", "flex ltd", "flextronics", "hp inc", "dyson",
        "razer", "delta electronics", "schneider electric", "siemens", "abb", "honeywell",
        "emerson", "rockwell", "continental", "bosch", "denso", "aptiv", "harman", "garmin",
        "semiconductor", "wafer", "photonics", "foundry", "silicon", "asic", "fpga",
        "utac", "ase group", "sk hynix", "samsung", "kioxia", "solidigm", "sandisk", "pure storage",
        "netapp", "arista", "juniper", "nokia", "ericsson",
    ]),
]

# 只在公司名里匹配的词(品牌名);这几个行业通名才允许在职位名里匹配
GENERIC_OK = {
    "semiconductor", "wafer", "photonics", "foundry", "asic", "fpga",
}
...
def push_telegram(new_jobs, cfg):
    ...
    if not token or not chat:
        ...
        return
    lines = [f"<b>新加坡 IT 新职位 {len(new_jobs)} 条</b>"]
...
def main():
    ap = argparse.ArgumentParser(description="新加坡 IT 职位每日差分监控")
